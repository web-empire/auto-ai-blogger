...
class WhatsApp_Handler {
	...
	private function send_whatsapp_message( $phone_number, $message ): bool {
		try {
			...
			if ( empty( $license ) ) {
				error_log( 'WP AI Blogger: No license key found for WhatsApp notification.' );
				return false;
			}
			...
			if ( is_wp_error( $response ) ) {
				error_log( 'WP AI Blogger WhatsApp Error: ' . $response->get_error_message() );
				return false;
			}
			...
			if ( $http_code !== 200 ) {
				...
				error_log( sprintf( 'WP AI Blogger WhatsApp Error: HTTP %d - %s', $http_code, $body ) );
				return false;
			}
			...
			error_log( 'WP AI Blogger WhatsApp Error: ' . ( $decoded['message'] ?? 'Unknown error' ) );
			return false;

		} catch ( \Exception $e ) {
			error_log( 'WP AI Blogger WhatsApp Exception: ' . $e->getMessage() );
			return false;
		}
	}
